...
static void mask_set(uint8_t* mask, uint8_t index) //bit a 1
{
  *mask |= (0x1<<index);
  return;
}

static void mask_clear(uint8_t* mask, uint8_t index) //bit a 0
{
  *mask &= ~(0x1<<index);
  return;
}
...
struct Timer* Timer_create(uint16_t duration_ms, TimerFn timer_fn, void* timer_args) 
{
  ...
  return timers; // Non ci sono slot disponibili
}
